...
1. Sign in to <https://pypi.org/manage/project/excitertools/settings/publishing/>.
2. Add a new GitHub trusted publisher with:
   - Owner: `cjrh`
   - Repository: `excitertools`
...
For a brand-new PyPI project, use PyPI's pending publisher form instead:
...
Note: this project historically uses CalVer (for example, `2025.0.1`).
`uv version --bump patch` increments the trailing component. For a calendar
rollover (`2025.0.1` → `2026.1.0`), edit `pyproject.toml` by hand and commit,
then tag or run the remaining release steps manually.
